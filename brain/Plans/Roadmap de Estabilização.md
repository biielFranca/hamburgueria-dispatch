...
### 1.3 Tirar o polling de integrações do cliente — 🟡 iFood por webhook no ar, falta cadastrar no portal (26/set/2026)
...
> **Pendente:** (1) dono cadastra `https://cuvhtdtkuwewslozddfw.supabase.co/functions/v1/ifood-webhook` no Portal do Desenvolvedor e faz um pedido de teste; (2) depois de confirmado, `IfoodPoller` deixa de fazer polling (vira indicador de status + botão "sincronizar agora") — presença por polling e por webhook não podem coexistir; (3) Open Delivery: cron de reconciliação; (4) cron do `compute-alert-state`.

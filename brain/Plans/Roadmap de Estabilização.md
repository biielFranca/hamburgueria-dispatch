...
> **✅ Alerta de atraso agendado (26/set):** job `pg_cron` `recompute-alert-levels` a cada 30 s chama `private.recompute_all_alert_levels()` direto no banco — sem Edge Function, sem egress. Migration `20260926h_alert_level_cron.sql`. Corrigido antes de agendar: (a) a função regravava `alert_level` de todo pedido aberto a cada execução (com Realtime ligado, cada regravação vira `fetchAll()` na Operacional) — agora só grava quando o nível muda; (b) tratava `external_monitoring` como status, mas é `route_eligibility` — pedido com entrega do iFood e retirada levariam "Pedido atrasado!" em 10 min; (c) saiu de `public`/`SECURITY DEFINER` para `private` (advisor: 5 → 4, só os helpers das policies). Testado com rollback: 6 min → `warning`, repetir → 0 linhas, 9 min → `urgent`, 11 min → `overdue`, despachado → limpa, entrega iFood com 30 min → sem alerta. Primeira execução do cron: 4 ms. Limiares mantidos (5/9/10 min desde o pedido).
> **`compute-alert-state` aposentada:** apagada do repo e do `scripts/verify-edge-auth.ps1`. **Pendente (dono):** apagar do dashboard (Edge Functions → compute-alert-state → Delete).
> **99Food por webhook (Decisão 015, 26/set):** a 99 tem API própria ("99Food Protocol", recomendada por ela) com webhook; a camada Open Delivery dela é **só polling**. Nova função `food99-webhook` v1: valida `didi-header-sign` (MD5 do corpo bruto + `app_secret`), lê IDs de 64 bits sem perder precisão, trata `orderNew`/`orderCancel`/`orderFinish`, responde `{"errno":0}`. O `orderNew` traz o pedido completo, então receber não exige `auth_token`. Regras puras em `_shared/food99.ts` (12 testes com o exemplo da documentação). 99 removida do `OpenDeliveryPoller`; card de Configurações atualizado. Credenciais em `store_integrations` (`99food`): `client_id` = app_id, `client_secret` = app_secret. Verificado: GET → 405; sem assinatura / assinatura falsa → 401.
> **Pendente 99:** (1) dono cria o app na 99 (aprovação em até 3 dias úteis) com webhook `https://cuvhtdtkuwewslozddfw.supabase.co/functions/v1/food99-webhook`, grava app_id/app_secret pelo dashboard e ativa a integração; (2) teste com pedido real (caminho positivo ainda não testado — não há secret); (3) vincular a loja ao app (página de autorização da 99 ou time da 99) — usar o `store_id` como `app_shop_id` evita depender da descoberta automática; (4) a loja confirma pedidos no app da 99 (B-App) em até 5 min, senão a 99 cancela; (5) confirmação de despacho de entrega própria ainda chama o caminho Open Delivery (`open-delivery-dispatch-confirm`) e vai falhar para a 99 — reescrever pelo protocolo próprio (exige `auth_token` por loja).
> **Keeta por webhook (Decisão 016, 26/set):** o Keeta suporta webhook no próprio Open Delivery (`POST {base}/v1/newEvent`), mas o `open-delivery-webhook` antigo era incompatível (pedia `?platform&storeId` na URL, lia outro cabeçalho, comparava hex sobre o corpo re-serializado, respondia 200 para assinatura inválida) e **logava o corpo inteiro** (dados do cliente). Nova função `keeta-webhook` v1: loja pelo `X-App-MerchantId`; `X-App-Signature` = Base64(HMAC-SHA256(client_secret, URL & params & corpo RFC 8785)) — testa as variantes de URL/corpo porque a doc não fixa qual URL o Keeta assina (o runtime entrega `req.url` interno, `http://…/keeta-webhook/…`, então a URL pública é montada a partir de `SUPABASE_URL`); busca o pedido pela `orderURL` com token `app_level_token` e requisição assinada; descriptografa os campos `ENC_` só em entrega própria (é o único caso em que o Keeta permite) e nunca grava texto cifrado; responde 204. Regras puras em `_shared/keeta.ts` (15 testes, incluindo os exemplos de assinatura da doc). `OpenDeliveryPoller` e `open-delivery-webhook` removidos do repo. Verificado: GET → 405; sem assinatura / assinatura falsa → 401.
> **Aceite automático (decisão do dono, 26/set):** no Keeta o sistema integrado tem de chamar `POST /v1/orders/{id}/confirm` em até 5 min, senão o Keeta cancela e **fecha a loja temporariamente**. `keeta-webhook` v2 confirma todo pedido novo assim que grava (`orderExternalCode` = nosso `orders.id`); se a confirmação falhar responde 500, o Keeta reenvia o `CREATED` e só a confirmação é refeita (marcada em `idempotency_keys`); pedido que já nasce cancelado não é confirmado.
> **Pendente Keeta:** (1) confirmar no primeiro pedido real que o aceite automático chega ao Keeta (evento `CONFIRMED`); (2) dono cadastra a URL base `https://cuvhtdtkuwewslozddfw.supabase.co/functions/v1/keeta-webhook` no portal do Keeta, grava client_id/client_secret pelo dashboard e ativa a integração; (3) teste com pedido real (caminho positivo não testado — sem secret; o primeiro pedido confirma qual variante de assinatura o Keeta usa); (4) apagar `open-delivery-webhook` pelo dashboard; (5) limpeza: `open-delivery-sync`, `open-delivery-dispatch-confirm`, `_shared/openDelivery.ts` e `src/lib/integrations/openDelivery.ts` ficam sem uso real quando a confirmação de despacho da 99 e do Keeta for reescrita.
...
> **Verificado pelo dono** com pedidos reais do iFood entrando pelo webhook: som, card e notificação do Windows. O alerta de **atraso** passou a rodar em 26/set (job `recompute-alert-levels`, ver 1.3); vale só para pedidos que a loja despacha (entrega própria).
